    ...
    class PlayerCharacter :BaseCharacter, IMonoBehaviour
    {
        ...
        private void Shoot(float deltaTime)
        {
            ...
            if (Engine.GetKey(Keys.SPACE) && shootTimer <= 0)
            {
                ...
                var bullet = Program.bullets.GetObjectFromPool();
                Program.bullets.AddToPool(bullet);

                if (bullet == default)
                {
                    bullet = new Bullet(b, true);
                }

                else
                {
                    bullet.Reset(transform.position.x, transform.position.y, transform.rotation);
                }
            }
